export const strategicFetch = function(
    input: RequestInfo,
    options?: RequestInit,
    strategy?: Strategy
): Promise<Response> {
    ...
    if (strategy) {
        ...
        delay = getDelay(strategy.retryPolicy);
    ...
    return new Promise((resolve, reject) => {
        ...
        const attempt = () => {
            if (internalLoggingEnabled) {
                console.log(
                    `${currentTime()} Strategic Fetch Attempt for Request to ${input.toString()}`,
                    count
                );
            }
            return fetch(input, options)
                .then(response => {
                    if (!response.ok) {
                        if (internalLoggingEnabled) {
                            console.log(
                                `${currentTime()} Response was not ok: `,
                                response.status
                            );
                        }
                        if (
                            softRejectCodes.indexOf(response.status) > -1 &&
                            count < retries
                        ) {
                            count++;
                            if (count > 1) {
                                delay = getDelay(retryPolicy, count);
                                if (internalLoggingEnabled) {
                                    console.log(
                                        `${currentTime()} Retrying with delay: `,
                                        delay
                                    );
                                }
                            }
                            ...
                            hardRejectCodes.indexOf(response.status) > -1
                        ) {
                            if (logProvider) {
                                logProvider.logFailure(
                            ...
                            reject(
                                new Error(
                                    'HTTP Error Occured:' + response.status
                                )
                            );
                        }
                    } else {
...
const getDelay = (retryPolicy: RetryPolicy, retryCount?: number): number => {
...
export class DefaultLogProvider implements LogProvider {
    logSuccess = (options: object) => {
        console.log(
            `DefaultLogProvider Network Call Success Details :: ${JSON.stringify(
                options
            )} `
        );
    };
    ...
    logFailure = (message: string, options: object): void => {
        console.log(
            `DefaultLogProvider Network Call Failure : Message :: ${message}, Details :: ${JSON.stringify(
                options
            )} `
        );
    };
